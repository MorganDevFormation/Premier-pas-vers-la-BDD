<script>
  ...
  const handleDrop = (e) => {
    ...
    const listHoldingCard = lists.find((list) =>
  ...
  const editList = async (e) => {
    ...
    try {
      ...
      form.error =
        "Une erreur s'est produite lors de la mise à jour de la liste.";
    }
  };
  ...
  const removeList = () => {
    try {
      ...
      form.error =
        "Une erreur s'est produite lors de la suppression de la liste.";
    }
  };
