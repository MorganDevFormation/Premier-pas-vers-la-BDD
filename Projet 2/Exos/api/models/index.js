...
User.belongsTo(Role,
	// configuration de l'association
	{
		...
		foreignKey: 'role_id',
		// alias du nom de la relation entre User et Role
		// belongs to 1 Role ==> donc alias = role au singulier
		as: 'role',
	});
...
Role.hasMany(User,
	{
		// nom de la colonne qui contient la clé étrangère 
		// la même que belongsTo pour User
		foreignKey: 'role_id',
		// alias : has many User ==> alias = users
		// plusieurs User, donc users au pluriel
		as: 'users',
	}
);
...
export { List, Card, Tag, User, Role };